...
cv2.createTrackbar("Scale", "Color Adjustments", 0, 255, nothing) #color contain only 256 value 
...
#convert gray scale to pencil effect 
while True:
    ...
    #Step - 3
    inverted_gray =  clr - gray  #inverted color image(shading effect )
    #Step -4
    blur_img = cv2.GaussianBlur(inverted_gray,(21,21),0) #pencil effect
    #Step -5
    ...
    #Output-----------------------
    cv2.imshow("opt",fltr)
    k = cv2.waitKey(1) #Dynamicaly perform
    if k == ord("q"):
         break
    if k == ord("s"):
